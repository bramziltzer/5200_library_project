...
-- add book
  ...
  -- use transactions
DELIMITER $$
CREATE PROCEDURE add_book (isbn_p INT, title_p VARCHAR(64), author_id_p INT, genre_p VARCHAR(64), num_pages_p INT,
...
BEGIN
    ...
    (SELECT isbn, title FROM book WHERE title = search_content);
  ELSEIF search_type_p = 'author' THEN
  	SELECT * FROM book_copy WHERE isbn IN 
    (SELECT isbn, author_id FROM book_author WHERE author_id IN 
     (SELECT author_id, author_name FROM author WHERE author_name = search_content));
  ELSE
  	SELECT * FROM book_copy WHERE isbn = CAST(search_content AS UNSIGNED);
  END IF;
...
BEGIN
  ...
  INSERT INTO book_checkout(book_copy_id INT, 
...
--  search all late fees
DELIMITER $$
CREATE PROCEDURE search_all_late_fees ()
BEGIN
...
-- clear late fees
DELIMITER $$
CREATE PROCEDURE clear_late_fees (member_id_p INT, fee_amount_p INT)
clf: BEGIN
